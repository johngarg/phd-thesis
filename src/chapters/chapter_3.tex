...
{\color{red}Need to alter this discussion.} We use these results to guide our
...
\begin{figure}[t]
  \centering%
\centering \includegraphics[scale=0.455]{runrat.pdf}
...
\begin{figure}[t]
  ...
  \begin{minipage}[t]{0.45\linewidth}
    \centering \includegraphics[scale=0.55]{densityPlot.pdf}
  \end{minipage}
  \hfill
  \begin{minipage}[t]{0.45\linewidth}
    \centering \includegraphics[scale=0.55]{densityPlotZoom.pdf}
  \end{minipage}
...
\begin{equation} \label{eq:ch3-amu}
  a_\mu^{S_{1}} = \sum_{i} \frac{m_\mu m_{u_i}}{4\pi^2 m_{S_{1}}^2} \left(
    \frac{7}{4} - \ln \frac{m_{S_{1}}^2}{m_{u_i}^2} \right) \text{Re} % (y^*_{2i} z^*_{2 i})
...
to be $\mu = \overline{m_c} = 1.27 \text{ GeV}$ for the $D$ meson decays and $\mu = 2
...
\paragraph{Comments on lepton flavor universality in $B \to
...
\begin{figure}
  ...
  \begin{minipage}[t]{\linewidth}
    \centering \includegraphics[scale=0.14]{cllclrPts.png} \subcaption[The
    results of scan I (black) and scan II (red) projected onto the
    $C_{LL}^{S_{1}}$--$C_{LR}^{S_{1}}$ plane.]{{\color{red}Needs to be updated,
        original figure depricated.}The results of scan I (black) and scan II
      (red) projected onto the $C_{LL}^{S_{1}}$--$C_{LR}^{S_{1}}$ plane. The
      colored contours correspond to those in Fig.~\ref{fig:ch3-cllclr}: the
      orange represent the fit to only LFU observables while the blue take into
      account all $b\to s$ observables. The model can alleviate the tensions in
      LFU observables to just within the $1\sigma$ region, a significant
      improvement on the SM. In this region, $C_{LR}^{S_{1}} \approx 0$,
      implying a suppression of the $y_{2i}$. Agreement with all of the
      $b \to s$ data is not as good.\\}
    \label{fig:ch3-cllclrpts1}
  \end{minipage}
  \begin{minipage}[t]{.45\linewidth}
    \centering \includegraphics[scale=0.085]{cllm.png}
  ...
  \hfill
  \begin{minipage}[t]{.45\linewidth}
    \centering \includegraphics[scale=0.085]{x33x23rat.png}
  ...
  \caption[The key results probing the extent to which the model can explain the
    ...
    for leptoquark masses between $3$ and $5 \text{ TeV}$, $|x_{23}| > |x_{33}|$
    and suppressed $y_{2i}$. The grey areas in (b) and (c) are the $1$ and
